...
``` r
pbmc <- utils::getFromNamespace("pbmc3k", "pbmc3k.SeuratData")
...
returns them directly in the universal marker format consumed by
`gs_prepare()` (pass `format = "db"` for the legacy wide ScType-style
format). The default `method = "fc"` is dependency-light; `"seurat"`
and `"presto"` backends are also available:
