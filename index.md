...
- Learn marker weights from data with 7 methods: `uniform`,
  `correlation`, `lr`, `glmnet` (default), `rf`, `xgb`, `lrp`
- Cross-validated weight training (`cv_folds`) for stable, statistically
  sound weights
- Optional marker-sensitivity weighting and score normalization
...
Optional packages for the individual weight-learning methods: `glmnet`
(method `glmnet`), `ranger` (method `rf`), `xgboost` (method `xgb`),
`keras` + `innsight` (method `lrp`).
...
accepts log-normalized data by default (`scaled = FALSE`; it performs
its own z-scoring) or pre-scaled data with `scaled = TRUE`. When scoring
with learned weights (see below), use
`norm = "weight", use_sensitivity = FALSE` — this normalizes scores by
the total marker weight and avoids double-penalizing shared markers.

### Train marker weights

[`train_weights()`](https://pwwang.github.io/hitype/reference/train_weights.md)
learns per-marker weights from a labeled dataset (clusters or cell
types) with 7 backends:
...
optimistic, circular results. See
...
discovers marker genes from a labeled dataset and returns them directly
in the database format consumed by
