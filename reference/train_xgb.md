...
One booster per cell type (that cell type against all the others), so
the gain of a marker is learned for the cell type it is reported for
instead of being shared by every cell type.

## Usage

``` r
train_xgb(data, clusters, class_markers = NULL)
```
