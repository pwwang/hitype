...
To train the weights for the markers, firstly, we have to have the markers.
They can be in the universal marker format (a long table with `cell_type`
and `gene` columns — what `find_markers()` returns by default) or in the
original `ScType` format. Here we use the `ScType` format:
...
`pbmc$seurat_clusters`) and returns them directly in the universal marker
format consumed by `gs_prepare()` (pass `format = "db"` for the legacy
wide ScType-style format), with dependency-light fold-change (`fc`,
